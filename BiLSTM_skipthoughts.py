...
class BiLSTM(object):
    def __init__(self, batch_size, max_sentence_len, rnn_size, margin):
        ...
        self.inputQuestions = tf.placeholder(tf.float32, shape=[None, 1, 4800])
        self.inputTrueAnswers = tf.placeholder(tf.float32, shape=[None, 1, 4800])
        self.inputFalseAnswers = tf.placeholder(tf.float32, shape=[None, 1, 4800])
        self.inputTestQuestions = tf.placeholder(tf.float32, shape=[None, 1, 4800])
        self.inputTestAnswers = tf.placeholder(tf.float32, shape=[None, 1, 4800])
