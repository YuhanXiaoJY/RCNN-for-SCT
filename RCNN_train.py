...
def RCNN_train():
    ...
    train_story, train_true_ans, train_false_ans, val_story, val_true_ans, val_false_ans,\
        test_story, test_ans1, test_ans2 = load_data()
    ...
    def predict():
        ...
        for p_k in tqdm(range(test_batch_num)):
            ...
            p_story = np.array(p_story)
            p_ans1 = np.array(p_ans1)
            p_ans2 = np.array(p_ans2)
    ...
    def evaluate():
        ...
        for val_k in tqdm(range(val_batch_num)):
            ...
            e_story = np.array(e_story)
            e_true_ans= np.array(e_true_ans)
            e_false_ans = np.array(e_false_ans)
